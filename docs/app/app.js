...
document.addEventListener('keydown', (e) => {
  ...
  if (ctrl && e.key === 'k') {
    e.preventDefault();
    sharedInput.focus();
    sharedInput.selectionStart = sharedInput.selectionEnd = sharedInput.value.length;
  }
  // Ctrl/Cmd+S → export layout (without saving to file)
  if (ctrl && e.key === 's') {
    e.preventDefault();
    exportLayout();
  }
...
// Shortcuts scoped to sharedInput
sharedInput.addEventListener('keydown', (e) => {
  const ctrl = e.ctrlKey || e.metaKey;
  // Ctrl/Cmd+Enter → run compare explicitly
  if (ctrl && e.key === 'Enter') {
    e.preventDefault();
    runCompare();
  }
